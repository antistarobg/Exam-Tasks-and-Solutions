#
# Lab Environment for WSAA 2022.01 - M4 - Task 2 - WFC
#
...
# Log on to the HW42-SRV1 machine, open Failover Cluster Manager and examine the result

# Or execute all or some of the following commands
Invoke-Command -VMName $VM2 -Credential $DC -ScriptBlock { Get-ClusterNode -Cluster ClusterHW }
Invoke-Command -VMName $VM2 -Credential $DC -ScriptBlock { Get-ClusterResource }
Invoke-Command -VMName $VM2 -Credential $DC -ScriptBlock { Get-ClusterQuorum }
Invoke-Command -VMName $VM2 -Credential $DC -ScriptBlock { Get-ClusterSharedVolume }